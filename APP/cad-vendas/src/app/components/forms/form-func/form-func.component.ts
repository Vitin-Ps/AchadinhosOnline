...
export class FormFuncComponent implements OnInit, AfterContentInit {
  ...
  validaForm() {
    this.funcForm = new FormGroup({
      ...
      nome: new FormControl(this.funcData ? this.funcData.nome : '', [
        Validators.required,
      ]),
      email: new FormControl(this.funcData ? this.funcData.email : '', [
        Validators.required,
      ]),
      porcentagem: new FormControl(
        this.funcData ? this.funcData.porcentagem : '',
        [Validators.required]
      ),
  ...
  arquivoSelecionado(event: any) {
    ...
    this.funcForm.patchValue({ imagem: arquivo });
  }
  ...
  get imagem() {
    return this.funcForm.get('imagem')!;
  }
  ...
  submit() {
    ...
    if (
      ...
      this.mensagensService.alert(
        `Altere ao menos um dado de ${this.funcData.nome}!`
      );
      return;
    }
